...
|---|---|---|---|
| 2026-09-04 | 51d1575 | **Storage elevation SHIPPED:** per-child UAC via `ShellExecuteW(verb=runas)` on windows-sys 0.52. Flow: (1) unelevated smartctl probe first; (2) if no drives found, ONE elevated cmd chained across all 16 `PhysicalDriveN` writes JSON to temp file, we poll+read; (3) UAC-decline → empty string → UI hides Storage card silently. `HWND=0` (isize in 0.52, not `*mut c_void`). New dep: `windows-sys 0.52` (Win32_Foundation/UI_Shell/Win32_UI_WindowsAndMessaging). | E2E **33871051572** ✅ 5/5 WebDriver · artifact zip unchanged |
| 2026-09-04 | 69e91c1 | rustfmt fix battery.rs (3 diff spots) + storage.rs (1 diff) — CI pipeline was failing at `cargo fmt --check` | CI **33846798402** ✅ rustfmt + clippy + 38 tests |
| 2026-08-26 | 15004dc | **Ground-truth fixes #3 (desktop round):** (1) storage — `smartctl --scan` is unreliable on Windows (known issue: misses NVMe, empty without admin); now ALSO probes `\\.\PhysicalDrive0..15` directly, absent drives fail harmlessly; (2) battery — desktop = silent skip ("No battery present" filtered in useScan like "requires Windows"), no more error banner | E2E 32965408944 ✅ 5/5 · local tsc/vitest(8) ✅ |
